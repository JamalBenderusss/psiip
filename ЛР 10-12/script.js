...
function mathA(a,x){
  if (x>=2) {
      if ((Math.pow(x,2)-1,5) == 0) { alert("Деление на 0"); return 0;}
      if ((a+x) < 0) {alert("Отрицательное"); return 0;}

    p.innerHTML=((a/(Math.pow(x,2)-1,5))+ Math.sqrt(2+x));
    return (a/(Math.pow(x,2)-1,5))+ Math.sqrt(2+x); 
  }else
  if (x>=0 && x<=2){
      if (a < 0) {alert("Корень из отрицательного"); return 0;}
