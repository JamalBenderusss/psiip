...
addButton.addEventListener('click', (e) => {
    ...
    if (input.value.length > 0) {
        const newInput = input.value.trim(); // очищаем от пробелов
        ...
        localStorage.setItem(buf,input.value);
        setCookie(input.value,buf);
        buf++;
        div.innerHTML = `<p class="name">${input.value}</p>
                    <div class="buttons">
